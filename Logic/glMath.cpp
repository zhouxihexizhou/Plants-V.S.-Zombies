...
void glMath::CalculateNormal(glVector *vert,glVector *norm,int numRow,int numCol)
{
	//初始化法向量为0
	int nIndex=0,i,j;
	for(i=0;i<numRow;i++)	
...
void glMath::Dir2HPR(glVector dir,glVector up,glVector &hpr)		
{
	dir.Normalize();
	up.Normalize();
	...
	glVector dir_XoZ(dir.x,0,dir.z);
	dir_XoZ.Normalize();
	...
	//判断正负
	glVector dir_leftStd=dir_upStd.crossMul(dir);
	temp = dir_leftStd.dotMul(dir_up);
	if(temp<0) hpr.z=-hpr.z;
}
double glMath::InverseMatrix(double Matrix[],double inverseMatrix[])
{
	double imat[16];
	int i,j,k;
